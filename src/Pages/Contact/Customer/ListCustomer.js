...
const ListCustomer = () => {
    ...
    const columns = React.useMemo(() => [
        ...
        { Header: 'Comment', accessor: 'comment' },
        {
            ...
            Cell: ({ row }) => (
                <div>
                    <Link to={`/customer/${row.original.id}`} className="btn btn-primary btn-sm me-2"><i className='fa fa-pencil'></i></Link>
    ...
    const deleteContent = () => {
        ...
        const deleteform = () => {
            dataService.deleteexe(`customer/customer/${rowId}`)
                .then(response => {
                    console.log("SUCCESS- DELETED CUSTOMER", response.data);
                    initCustomers();
                    setDeleteVisible(false);
                })
                .catch(error => {
                    console.error("ERROR- DELETED CUSTOMER", error);
                })
        }
    ...
    const { getTableProps, getTableBodyProps, headerGroups, rows, prepareRow, } = useTable({ columns, data: customers }, useFilters, useSortBy);
    ...
    return (
        <div className="container-fluid">
            <div className="d-flex flex-row align-items-center justify-content-between py-3">
                ...
                <Link to="/customer" className="btn btn-sm btn-primary shadow-sm">Add Customer </Link>
            </div>
